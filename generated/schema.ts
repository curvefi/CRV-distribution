...
export class VirtualPrice extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id !== null, "Cannot save VirtualPrice entity without an ID");
    assert(
      id.kind == ValueKind.STRING,
      "Cannot save VirtualPrice entity with non-string ID. " +
        'Considering using .toHex() to convert the "id" to a string.'
    );
    store.set("VirtualPrice", id.toString(), this);
  }

  static load(id: string): VirtualPrice | null {
    return store.get("VirtualPrice", id) as VirtualPrice | null;
  }
